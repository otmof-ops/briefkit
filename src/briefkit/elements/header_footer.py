...
def _draw_header_footer(canvas, doc, state, brand=None):
    ...
    org_text = truncate_to_width(org, "Helvetica-Bold", 7, max_text_width * 0.5)
    ...
    if section_name:
        ...
        section_text = truncate_to_width(section_name, "Helvetica", 7, max_text_width * 0.5)
    ...
    canvas.setFont("Helvetica", 7)
    canvas.setFillColor(caption)
    date_text = truncate_to_width(date_str, "Helvetica", 7, max_text_width * 0.33)
    ...
    canvas.setFillColor(caption)
    if hf_doc_id:
        doc_id_text = truncate_to_width(hf_doc_id, "Helvetica", 7, max_text_width * 0.33)
        canvas.drawRightString(right, bottom + 1.5 * mm, doc_id_text)
        copyright_text = truncate_to_width(copyright, "Helvetica", 7, max_text_width * 0.5)
        ...
        copyright_text = truncate_to_width(copyright, "Helvetica", 7, max_text_width * 0.33)
...
def build_classification_banner(level, path, brand=None, content_width=None):
    ...
    banner_data = [
        [
            Paragraph(
                title_left,
                _ps("BannerTitle", brand=b, fontSize=11, textColor=white, fontName="Helvetica-Bold"),
            ),
            Paragraph(
        ...
        [
            Paragraph(
                ...
                _ps("BannerPath", brand=b, fontSize=8, textColor=white, fontName="Courier"),
            ),
            Paragraph(
