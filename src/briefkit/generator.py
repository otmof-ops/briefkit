...
class BaseBriefingTemplate:
    ...
    def build_back_cover(self) -> list:
        ...
        Uses brand config for org name, tagline, URL, and copyright.
        """
        b = self.brand
        ...
        top_data = [[
            Paragraph(
                f'<font color="white"><b>{_safe_text(org) if org else "briefkit"}</b></font>',
                _ps("BackCoverTop", brand=b, fontSize=12, textColor=white,
                    fontName="Helvetica-Bold"),
            ),
        ]]
        top_bar = Table(top_data, colWidths=[self.content_width], rowHeights=[16 * mm])
        ...
        if tagline:
            tagline_style = _ps(
                "BackCoverTagline", brand=b,
                fontSize=20, textColor=primary,
                fontName="Helvetica-Bold", alignment=1,
            )
            flowables.append(Paragraph(f'"{_safe_text(tagline)}"', tagline_style))
            flowables.append(Spacer(1, 8 * mm))

        if org:
            sub_style = _ps(
                "BackCoverSub", brand=b,
                fontSize=11, textColor=caption_c,
                fontName="Helvetica-Oblique", alignment=1,
            )
            flowables.append(Paragraph(_safe_text(org), sub_style))
            flowables.append(Spacer(1, 6 * mm))

        if url:
            url_style = _ps(
                "BackCoverURL", brand=b,
                fontSize=9, textColor=secondary,
                fontName="Courier", alignment=1,
            )
            flowables.append(Paragraph(_safe_text(url), url_style))
            flowables.append(Spacer(1, 20 * mm))

        meta_style = _ps(
            "BackCoverMeta", brand=b,
            fontSize=8, textColor=caption_c, alignment=1,
        )
        flowables.append(Paragraph(
            f"This document was generated on {self.date_str}.",
            meta_style,
        ))
        flowables.append(Paragraph(copyright_str, meta_style))
        flowables.append(Spacer(1, 40 * mm))

        bottom_data = [[
            Paragraph(
                '<font color="white">Generated by briefkit</font>',
                _ps("BackCoverBottom", brand=b, fontSize=7, textColor=white),
            ),
        ]]
        bottom_bar = Table(bottom_data, colWidths=[self.content_width], rowHeights=[10 * mm])
    ...
    def generate(self) -> Path:
        ...
        # Sync module-level _hf_state for templates that mutate it directly
        _hf_state["section"] = hf_state["section"]
        _hf_state["date"]    = hf_state["date"]
        _hf_state["doc_id"]  = hf_state["doc_id"]
        ...
        if not variant_name:
            ...
            all_text = content.get("overview", "") + " " + content.get("title", "")
            for sub in content.get("subsystems", []):
